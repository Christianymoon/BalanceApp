...
class ActiveSection:
            ...
            self.active_list.controls.append(
                ft.Container(
                    content=ft.Row([
                        ...
                        ft.Container(
                            content=ft.Icon(
                                icons.CHECK_CIRCLE if item[4] else icons.ERROR, color=self.theme.green_color if item[4] else self.theme.text_secondary, size=20),
                            width=30,
                            height=20
                        ),
                        ft.Column([
                            ...
                            ft.Text(
                                "Liquido" if item[4] else "No Liquido", color=self.theme.green_color if item[4] else self.theme.text_secondary, size=12)
    ...
    def _get_total_sum(self, list_items):
        return sum(item[3] for item in list_items)

    def refresh_actives(self):
        self.active_list.controls.clear()
        actives_list = ActiveController.controller_fetch_actives()
        self.total_actives = f"${self._get_total_sum(actives_list):,.2f}"
        self.add_active_list(actives_list)
        self.page.update()
    ...
    def draw(self, header):
        ...
        self.liquid_checkbox = ft.Checkbox(
            label="Es liquido", value=False, label_style=ft.TextStyle(color=self.theme.text_primary), check_color=self.theme.bg, active_color=self.theme.text_primary)
