...
class IntertransferView:
    ...
    def intertransfer(self, e):
        try:
            ActiveController.controller_intertransfer(
                self.temp_account_from, self.temp_account_to, self.quantity_field.value)
            self.page.go("/active")
    ...
    def validate_quantity(self, e):
        ...
        self.total_account = ActiveController.controller_fetch_active(
            self.temp_account_from)
            ...
            self.intertransactions_container.content.controls.append(
                ft.Container(
                    content=ft.ResponsiveRow([
                        ...
                        ft.Row([
                            ft.Text(
                                f"$ {intertransaction[3]:.2f}", color=self.theme.green_color, size=12),
                            ft.Text(
                                f"{intertransaction[4]}", color=self.theme.text_primary, size=12),
                        ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),

                        ft.Row([
                            ft.Text(
                                f"{intertransaction[1]}", color=self.theme.text_primary, size=12),
                            ft.Icon(icons.SYNC_ALT,
                                    color=self.theme.green_color, size=20),
                            ft.Text(
                                f"{intertransaction[2]}", color=self.theme.text_primary, size=12),
    ...
    def draw(self, header: HeaderSection):
        ...
        self.header = header.create(
            "Intertransferencia de Activos", return_page=True)

        self.hint_text = ft.Text(
            "Selecciona la cuenta origen y la cuenta destino", color=self.theme.text_primary, size=12)
        ...
        for account in self.accounts:
            if account[1] == "Prestamo":
                continue  # TODO: Remove this condition when loans can also be source of intertransfers
        ...
        for account in self.accounts:
            if account[1] == "Prestamo":
                continue  # TODO: Remove this condition when loans can also be destination of intertransfers
        ...
        self.transfer_button = ft.Container(
            content=ft.Row([ft.Icon(icons.SEND, color=self.theme.bg), ft.Text(
                "Intertransferir", color=self.theme.bg, size=16, weight=ft.FontWeight.BOLD)], alignment=ft.alignment.center),
