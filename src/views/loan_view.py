...
class LoanView:
    ...
    def liquidate(self, e):
        ...
        try:
            LoanController.liquidate(
                self.data["loan_id"], self.current_loan[1], account_id)
            self.page.go("/")
    ...
    def draw(self, header):
        ...
        self.header = header.create(
            placeholder="Prestamos", return_page=True, config=True)

        self.card = ft.Container(
            ft.Column([
                ft.Text(self.current_loan[2].capitalize(
                ...
                ft.Row(
                    [
                        ft.Icon(icons.CHECK_CIRCLE if self.current_loan[7] else icons.CANCEL,
                                color=self.theme.green_color if self.current_loan[7] else self.theme.red_color, size=20),
                        ft.Text(
                            "Cobrado" if self.current_loan[7] else "Aun sin cobrar", size=14)
                    ],
                    alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                    spacing=10
                ),
        ...
        self.hint_text = ft.Text(
            "Al cobrar el prestamo se sumara la cantidad a la cuenta seleccionada", color=self.theme.text_secondary, size=12)
        ...
        self.liquidate_button = ft.Container(
            content=ft.Text("Liquidar prestamo", color=self.theme.bg,
                            size=16, weight=ft.FontWeight.BOLD),
